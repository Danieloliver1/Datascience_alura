...
def load_model():
    ...
    with open('/mount/src/datascience_alura/Machine_Learning_com_Regressão/03_Series_Temporais/prevendo_series_temporais_com_Prophet/Aplicacao/modelo_o3_prophet.json', 'r') as file_in:
...
if 'precisao_feita' not in st.session_state:
...
if st.session_state.previsao_feita:
    ...
    # código omitido
    fig.update_layout({
        # Define o fundo da área do gráfico como branco
        'plot_bgcolor': 'rgba(255, 255, 255, 1)',
        # Define o fundo externo ao gráfico como branco
    ...
    tabela_previsao.columns = ['Data (Dia/Mês/Ano)', '03 (ug/m3)']
    tabela_previsao['Data (Dia/Mês/Ano)'] = tabela_previsao['Data (Dia/Mês/Ano)'].dt.strftime('%d-%m-%Y')
    tabela_previsao['03 (ug/m3)'] = tabela_previsao['03 (ug/m3)'].round(2)
    tabela_previsao.reset_index(drop=True, inplace=True)
    st.write(
        f'Tabela contendo as previsões de ozônio (ug/m3 para os próximos {dias} dias:)')
